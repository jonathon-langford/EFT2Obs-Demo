...
namespace Rivet {
  ...
  public:
    ...
    bool originateFrom(const Particle& p, const Particles& ptcls ) {
      ...
      for (auto ancestor: HepMCUtils::particles(prodVtx, Relatives::ANCESTORS)){ 
	for ( auto part:ptcls ) 
    ...
    /// @brief Whether p HepMCUtils::article p originates from p2
    ...
    bool hasChild(ConstGenParticlePtr ptcl, int pdgID) {
    ...
    bool hasParent(ConstGenParticlePtr ptcl, int pdgID) {
    ...
    HiggsClassification classifyEvent(const Event& event, const STXS::HiggsProdMode prodMode ) {
      ...
      cat.stage1_cat_pTjet25GeV = STXS::Stage1::UNKNOWN;
      cat.stage1_cat_pTjet30GeV = STXS::Stage1::UNKNOWN;
      cat.stage1_1_cat_pTjet25GeV = STXS::Stage1_1::UNKNOWN;
      cat.stage1_1_cat_pTjet30GeV = STXS::Stage1_1::UNKNOWN;
      cat.stage1_1_fine_cat_pTjet25GeV = STXS::Stage1_1_Fine::UNKNOWN;
      cat.stage1_1_fine_cat_pTjet30GeV = STXS::Stage1_1_Fine::UNKNOWN;
      ...
      int Nhiggs=0;
      //for ( ConstGenParticlePtr ptcl : Rivet::particles(event.genEvent()) ) {
      for ( ConstGenParticlePtr ptcl : HepMCUtils::particles(event.genEvent()) ) {
	...
	if ( isVH(prodMode) && !cat.V.genParticle()->end_vertex() )
          return error(cat,STXS::VH_DECAY_IDENTIFICATION,"Vector boson does not decay!");
    ...
    STXS::Stage1::Category getStage1Category(const STXS::HiggsProdMode prodMode,
                                             const Particle &higgs,
                                             const Jets &jets,
                                             const Particle &V) {
      ...
      else if (prodMode == STXS::BBH)
        return Category(BBH_FWDH + ctrlHiggs);
      else if (prodMode == STXS::TH)
        return Category(TH_FWDH + ctrlHiggs);
      return UNKNOWN;
    }

    /// @brief Stage-1.1 categorization
    STXS::Stage1_1::Category getStage1_1_Category(const STXS::HiggsProdMode prodMode,
					     const Particle &higgs,
					     const Jets &jets,
					     const Particle &V) {
    ...
    STXS::Stage1_1_Fine::Category getStage1_1_Fine_Category(const STXS::HiggsProdMode prodMode,
                         const Particle &higgs,
                         const Jets &jets,
                         const Particle &V) {
      using namespace STXS::Stage1_1_Fine;
      int Njets=jets.size(), ctrlHiggs = std::abs(higgs.rapidity())<2.5, fwdHiggs = !ctrlHiggs;
