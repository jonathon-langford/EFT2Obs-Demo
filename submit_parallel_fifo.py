...
sub_handle = "%s_run$(procID)_fifo"%opt.process
N_process = opt.nJobs
...
f_sub.write("executable          = parallel_fifo.sh\n")
f_sub.write("arguments           = %s %s $(procID) %s %g %g %g\n"%(os.environ['PWD'],opt.process,opt.nEvents,opt.disableReweight,opt.saveMG5RunDir,opt.saveHepMC))
